...
public class RoomHandler
{
    ...
    /// </summary>
    /// <param name="request"></param>
    public void HandlerGameStart(uint conv, GameStart request)
    {
        var room = _host.CurrentRoom;
        if (room == null) return;
    ...
    /// 获取当前请求的客户端 conv
    /// </summary>
    private uint _currentRequestConv;

    public void SetCurrentRequestConv(uint conv)
    {
        _currentRequestConv = conv;
    }

    private uint GetCurrentRequestConv()
    {
        return _currentRequestConv;
    }
